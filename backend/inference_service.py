...
from backend.postprocess import build_mask, calc_metrics, mask_to_base64
...
def run_inference(image_path: str, model_name: str) -> dict:
    ...
    try:
        ...
        logger.info(
            "推理完成 model=%s time_ms=%s lesion_area=%s lesion_ratio=%.6f",
            config["display_name"],
            elapsed_ms,
            lesion_area,
            lesion_ratio,
        ...
        _cleanup(outputs, tensor, mask, image_bgr)
